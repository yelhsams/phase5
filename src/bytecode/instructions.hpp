...
namespace bytecode
{
  enum class Operation
  {
    ...
    //                  f.local_reference_vars[i] : f.free_vars[i -
    ...
    // adheres to semantics of Assignment #2)
    ...
    // Operand i  (0 < i <= m) : free variable argument (m - i).
    // Argument (m - i) maps to position (m - i)
    // Operand n=m+1 : function
    // Stack:       S :: operand n :: ... :: operand 3 :: operand 2 :: operand 1
    // => S :: closure
    AllocClosure,
    ...
    // Result:      value of + operation as specified by the semantics of
    // Assignment #2
    ...
    // (semantics according to Assignment #2)
    // Mnemonic:    and/or
    // Operand 0:   N/A
    // Operand 1:   right value
    // Operand 2:   left value
    ...
    // Mnemonic:    goto i
    // Example: goto 0
    // jumps to the current instruction. goto 1 jumps to the following instruction.
    // goto -1 jumps to the preceding instruction
  ...
  // Register-based instruction format (coexists with legacy stack format)
  ...
  typedef std::vector<RegisterInstruction> RegisterInstructionList;
} // namespace bytecode
