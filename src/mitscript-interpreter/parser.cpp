...
bool mitscript::Parser::is_binary_op(mitscript::TokenKind k) const {
    ...
    switch (k) {
        case TK::ADD: case TK::SUB: case TK::MULT: case TK::DIV:
        case TK::LT: case TK::LE: case TK::GE: case TK::GT:
        case TK::EQEQ:
        case TK::AMP: case TK::BAR:
            return true;
...
int mitscript::Parser::precedence_of(mitscript::TokenKind k) const {
    ...
    // higher = tighter
    switch (k) {
    case TK::BAR:                return 1; // |
    case TK::AMP:                return 2; // &
    // Unary '!' should bind tighter than '|'/'&' but looser than comparisons/equality.
    case TK::BANG:               return 3; // ! (used for guiding unary parse)
    case TK::EQEQ:               return 4; // ==
    case TK::LT: case TK::LE: case TK::GE:
    case TK::GT:                 return 4; // < <= >= >
    case TK::ADD: case TK::SUB:  return 5; // + -
    case TK::MULT: case TK::DIV: return 6; // * /
        default: return -1;
    }
}

static inline mitscript::BinOp token_to_binop(mitscript::TokenKind k) {
    using TK = mitscript::TokenKind; using BO = mitscript::BinOp;
    switch (k) {
        case TK::ADD: return BO::ADD;  case TK::SUB: return BO::SUB;
        case TK::MULT: return BO::MUL; case TK::DIV: return BO::DIV;
        case TK::LT: return BO::LT;    case TK::LE: return BO::LTE;
        case TK::GE: return BO::GTE;   case TK::GT: return BO::GT;
        case TK::EQEQ: return BO::EQ;  case TK::AMP: return BO::AND;
...
[[noreturn]] void mitscript::Parser::error_here(const std::string &msg) const {
    const auto &t = peek(0);
    throw std::runtime_error("Parser error at line " + std::to_string(t.start_line) + ", col " + std::to_string(t.start_col) + ": " + msg);
}
...
std::unique_ptr<mitscript::Statement> mitscript::Parser::parse_statement() {
    if (check(TokenKind::LBRACE, 0)) return parse_block();
    if (check(TokenKind::IF, 0))     return parse_if_statement();
    if (check(TokenKind::WHILE, 0))  return parse_while_loop();
    if (check(TokenKind::RETURN, 0)) return parse_return_statement();
    if (check(TokenKind::GLOBAL, 0)) return parse_global_statement();
...
std::unique_ptr<mitscript::Global> mitscript::Parser::parse_global_statement() {
    ...
    auto token = expect(TokenKind::IDENTIFIER, "Expected identifier after 'Global'");
    ...
    if (match(TokenKind::FUN)) {
        ...
        if (!check(TokenKind::RPAREN, 0)) {
            ...
            while (match(TokenKind::COMMA)) {
                if (check(TokenKind::RPAREN, 0)) break;
        ...
        fn->name = ""; fn->args = std::move(params); fn->body = std::move(body);
        return fn;
    }
...
std::unique_ptr<mitscript::Expression> mitscript::Parser::parse_binary_rhs(int min_prec) {
    auto lhs = parse_simple_atom();
    while (is_binary_op(peek(0).kind) && precedence_of(peek(0).kind) >= min_prec) {
        auto opk = peek(0).kind;
        int prec = precedence_of(opk);
        advance();
        auto rhs = parse_binary_rhs(prec + 1);
...
std::unique_ptr<mitscript::Expression> mitscript::Parser::parse_simple_atom() {
    ...
    // literals
    if (match(TokenKind::INT))    { auto c = std::make_unique<mitscript::IntegerConstant>(); c->value = std::stoi(previous().text); return c; }
    if (match(TokenKind::STRING)) { auto c = std::make_unique<mitscript::StringConstant>();  c->value = decode_string_literal(previous().text);            return c; }
    if (match(TokenKind::TRUE))   { auto c = std::make_unique<mitscript::BooleanConstant>(); c->value = true;                       return c; }
    if (match(TokenKind::FALSE))  { auto c = std::make_unique<mitscript::BooleanConstant>(); c->value = false;                      return c; }
    if (match(TokenKind::NONE))   { return std::make_unique<mitscript::NoneConstant>(); }
...
std::unique_ptr<mitscript::Expression> mitscript::Parser::parse_unary_expression() {
    if (match(TokenKind::SUB)) {
        ...
        // Parse the operand as a binary expression with minimum precedence equal to '!'s precedence.
...
std::unique_ptr<mitscript::Expression> mitscript::Parser::parse_location() {
    ...
    std::unique_ptr<mitscript::Expression> var = std::make_unique<mitscript::Variable>();
