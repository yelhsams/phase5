...
template <typename T>
std::string joinList(const std::vector<T> &items, const char *prefix = "",
                     const char *suffix = "") {
  if (items.empty())
    return "[]";
...
std::string operandToString(const IROperand &operand) {
  switch (operand.kind) {
  case IROperand::VREG:
    return "v" + std::to_string(operand.i);
  case IROperand::LOCAL:
    return "local#" + std::to_string(operand.i);
  case IROperand::NAME:
    return "name(" + operand.s + ')';
  case IROperand::CONSTI:
    return "const(" + std::to_string(operand.i) + ')';
  case IROperand::CONSTS:
    return "const(\"" + operand.s + "\")";
  case IROperand::CONSTB:
    return std::string("const(") + (operand.i ? "true" : "false") + ')';
  case IROperand::NONE:
    return "None";
  }
  return "<operand>";
...
void printTerminator(const Terminator &term, std::ostream &os, int baseIndent) {
  ...
  case Terminator::Kind::CondJump:
    os << "cond v" << term.condition << " ? " << term.trueTarget << " : "
       << term.falseTarget;
    break;
...
void prettyprint(const FunctionCFG &fn, std::ostream &os, int indentLevel) {
  ...
  os << ind << "FunctionCFG\n";
  auto printStringList = [&](const char *label,
